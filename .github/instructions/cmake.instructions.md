...
- Follow "Modern CMake" best practices (https://cliutils.gitlab.io/modern-cmake/) to inform conventions, formatting and style.
- Put `cmake_minimum_required(VERSION 3.28)` at the top of the root `CMakeLists.txt`, and `project(...)` immediately after.
...
- Be explicit about dependencies, requirements, and configurations; avoid implicit global state.
- Be consistent and uniform in style, naming, structure and layout across all CMake files in the project.
  All app CMakeLists.txt should follow the same structure and layout; all library CMakeLists.txt should follow the same structure and layout, etc.
- Boilerplate and repetitive patterns should be encapsulated in reusable functions/macros/modules that are used consistently and uniformly.
...
- Enable warnings-as-errors by default. Clean builds should produce zero warnings.

## Options, Cache Variables, And Naming

- Use `option()` for user-togglable features; prefer cache variables over ad-hoc globals.
- Prefix project-defined cache variables with a project namespace (e.g. for a project MyProj use `MYPROJ_...`).
